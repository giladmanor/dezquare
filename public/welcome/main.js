...
;(function(window) 
{
	...
	Dezquare.Global = function() 
	{
		...
		var BindEvents = function()
		{
			...
			gameIcons.find("a.nosoon").click(function(e){
				e.preventDefault();
				gameIcons.find("a.nosoon").removeClass("active");
			...
			portfolioGallery.find("li").each(function(){
				var carouselHeight = portfolioGallery.height();
				if ($(this).height() > portfolioGallery.height())
				{
					portfolioGallery.height($(this).height());
				}
			});
		...
		/* End Base Functions */
	//$('.soon').removeClass('c1 c2 c3');
		$('.soon').removeAttr('href');
		$('.soon').removeAttr('onclick');
		$('.soon img').unbind('click');
